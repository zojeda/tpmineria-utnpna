...
```{r}
sentencias_1c <- sentencias_1c %>% 
  mutate(loc_Gualeguaychu = localidad == 'Gualeguaychú') %>% 
  mutate(loc_Rosario_del_Tala  = localidad == 'Rosario del Tala' ) %>% 
  mutate(loc_Parana = localidad == 'Paraná') %>% 
  mutate(loc_Concepcion_del_Uruguay = localidad == 'Concepción del Uruguay') %>% 
  mutate(loc_Concordia = localidad == 'Concordia') %>% 
  mutate(loc_Federal = localidad == 'Federal') %>% 
  mutate(loc_Diamante = localidad == 'Diamante') %>% 
  mutate(loc_Colon = localidad == 'Colón') %>% 
  mutate(loc_Victoria = localidad == 'Victoria') %>% 
  mutate(loc_La_Paz = localidad == 'La Paz') %>% 
  mutate(loc_Gualeguay = localidad == 'Gualeguay') %>% 
  mutate(loc_Nogoya = localidad == 'Nogoyá') %>% 
  mutate(loc_Chajari = localidad == 'Chajarí') %>% 
  mutate(loc_Villaguay = localidad == 'Villaguay') %>% 
  mutate(loc_San_Salvador = localidad == 'San Salvador') %>% 
  mutate(loc_Federacion = localidad == 'Federación') %>% 
  mutate(loc_Feliciano = localidad == 'Feliciano') %>% 
  mutate(loc_Villa_Paranacito = localidad == 'Villa Paranacito')
...
```{r}
sentencias_1c <- sentencias_1c %>% 
  mutate(materia_fam_pen = materia == 'fam|pen') %>% 
  mutate(materia_paz  = materia == 'paz' ) %>% 
  mutate(materia_fam = materia == 'fam') %>% 
  mutate(materia_cco = materia == 'cco') %>% 
  mutate(materia_cco_lab = materia == 'cco|lab') %>% 
  mutate(materia_cco_fam_lab = materia == 'cco|fam|lab') %>% 
  mutate(materia_cco_lab_pen = materia == 'cco|lab|pen')
...
sentencias_bool <- select(sentencias_1c, -nro, -tproc, -as, -ccon, -finicio, -fres, -fdesp, -fvenc1, -fvenc2, -tres, -mat, -justiciables, -reccap, -iep, -duracion, -mediana_duracion, -circunscripcion, -localidad, -materia)
